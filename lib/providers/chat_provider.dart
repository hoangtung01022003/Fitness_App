...
import 'package:fitness/hive/boxes.dart';
import 'package:fitness/hive/chat_history.dart';
import 'package:fitness/hive/settings.dart';
import 'package:fitness/hive/user_model.dart';
...
class ChatProvider extends ChangeNotifier {
  ...
  Future<Content> getContent({
    ...
    if (isTextOnly) {
      ...
      return Content.text(message); // Sử dụng Content từ google_generative_ai
    } else {
      ...
      if (imageFutures == null || imageFutures.isEmpty) {
        throw Exception('No images provided for multi-content generation');
      }

      final imageBytes = await Future.wait(imageFutures);
      final prompt =
          TextPart(message); // Sử dụng TextPart từ google_generative_ai
      final imageParts = imageBytes
          .map((bytes) => DataPart(
              'image/jpeg',
              Uint8List.fromList(
                  bytes))) // Sử dụng DataPart từ google_generative_ai
          .toList();

      return Content.multi(
          [prompt, ...imageParts]); // Sử dụng Content từ google_generative_ai
    }
  }
